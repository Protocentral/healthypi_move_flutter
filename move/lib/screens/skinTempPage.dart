...
class _SkinTemperaturePageState extends State<SkinTemperaturePage>
    ...
    if (downloadsDirectory != null) {
      ...
      if (downloadsDir.existsSync()) {
        ...
        List<File> csvFiles = files
            ...
            .where((file) => p.basename(file.path).startsWith("temp_")) // Filter by prefix
            .toList();
  ...
  Future<void> getFileData(String fileName) async {
    ...
    maxTemp = result.map((f) => f.split(",")[1]).toList();

    minTemp = result.map((f) => f.split(",")[2]).toList();

    avgTemp = result.map((f) => f.split(",")[3]).toList();

    latestTemp = result.map((f) => f.split(",")[4]).toList();
    ...
    for(int i = 1; i< timestamp.length; i++){
      ...
      int tempMinTemp = 0;
      double tempMinTemp1 = 0;
      int tempMaxTemp = 0;
      double tempMaxTemp1 = 0;
      int tempAvgTemp = 0;
      double tempAvgTemp1 = 0;
      int tempLatestTemp = 0;
      double tempLatestTemp1 = 0;

      tempTimeStamp = int.parse(timestamp[i]);
      tempTimeStamp1 = tempTimeStamp*1000;
      tempMinTemp = int.parse(minTemp[i]);
      tempMinTemp1 = (tempMinTemp/100);
      tempMaxTemp = int.parse(maxTemp[i]);
      tempMaxTemp1 = tempMaxTemp/100;
      tempAvgTemp = int.parse(avgTemp[i]);
      tempAvgTemp1 = tempAvgTemp/100;
      tempLatestTemp = int.parse(latestTemp[i]);
      tempLatestTemp1 = tempLatestTemp/100;

      setState((){
        //print(DateTime.fromMillisecondsSinceEpoch(tempTimeStamp1).toString());
        TempTrendsData.add(TempTrends(DateTime.fromMillisecondsSinceEpoch(tempTimeStamp1),
            tempMinTemp1, tempMaxTemp1));
        if( i == timestamp.length-1){
          averageTemp = tempAvgTemp1;
          restingTemp = tempLatestTemp1;
          rangeMinTemp = tempMinTemp1;
          rangeMaxTemp = tempMaxTemp1;
                          ...
                          child: Column(
                              ...
                              children: <Widget>[
                                ...
                                Wrap(
                                  ...
                                  children: <Widget>[
                                    Text(rangeMinTemp.toString(), style: hPi4Global.moveValueTextStyle),
                                    Text('-', style: hPi4Global.moveValueTextStyle),
                                    Text(rangeMaxTemp.toString(), style: hPi4Global.moveValueTextStyle),
                                  ],
                                ),
                                Row(
                                  children: <Widget>[
