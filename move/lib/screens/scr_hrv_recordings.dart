...
/// Represents a single HRV recording session
class HRVRecording {
  final int sessionId;
  ...
  static String _formatSampleCount(int count) {
    if (count >= HRVConstants.sampleCountThreshold) {
      return '${(count / HRVConstants.sampleCountThreshold).toStringAsFixed(1)}k';
    }
    return count.toString();
  }
}

/// Modern HRV recordings management screen using FsManager for downloads
class ScrHRVRecordings extends StatefulWidget {
  final String deviceMacAddress;

  const ScrHRVRecordings({super.key, required this.deviceMacAddress});

  @override
  State<ScrHRVRecordings> createState() => _ScrHRVRecordingsState();
}

class _ScrHRVRecordingsState extends State<ScrHRVRecordings> {
  ...
  Future<void> _loadRecordingsList() async {
    ...
    try {
      ...
      // Convert to HRVRecording objects
      final recordings = <HRVRecording>[];
      print('HRV Recordings: Converting ${_logHeaderList.length} headers to recording objects...');

      for (final header in _logHeaderList) {
        // sessionLength is the number of data bytes (no header bytes to subtract)
        // Calculate sample count: dataBytes / bytesPerSample
        final sampleCount = header.sessionLength ~/ HRVConstants.bytesPerSample;
        final durationSeconds = (sampleCount / HRVConstants.samplingRateHz).toInt();
        ...
        print('HRV Recordings:   - Session ${header.logFileID}: ${header.sessionLength} bytes = $sampleCount samples = ${durationSeconds}s');
        print('HRV Recordings:     Timestamp: ${header.logFileID}s → ${dt.toIso8601String()} (year: ${dt.year})');

        recordings.add(HRVRecording(
      ...
      print('HRV Recordings: Recordings sorted. Displaying ${recordings.length} card(s)');
  ...
  Future<void> _fetchSessionCount() async {
    print('HRV Recordings: Fetching session count...');
    ...
    subscription = _dataCharacteristic!.onValueReceived.listen((value) {
      ...
      if (pktType == hPi4Global.CES_CMDIF_TYPE_CMD_RSP) {
        ...
        if (trendCode == hPi4Global.HRVRecord[0]) {
          _totalSessionCount = bdata.getUint16(3, Endian.little);
          print('HRV Recordings: Session count received: $_totalSessionCount');
          subscription.cancel();
    ...
    print('HRV Recordings: Sent HRVLogCount command');

    await completer.future.timeout(
      const Duration(seconds: 10),
      onTimeout: () {
        print('HRV Recordings: ⚠️ TIMEOUT waiting for session count');
        subscription.cancel();
        _activeSubscriptions.remove(subscription);
  ...
  Future<void> _fetchSessionIndices() async {
    if (_totalSessionCount == 0) {
      print('HRV Recordings: No sessions to fetch (count = 0)');
      return;
    }

    print('HRV Recordings: Fetching indices for $_totalSessionCount session(s)...');
    ...
    subscription = _dataCharacteristic!.onValueReceived.listen((value) {
      ...
      if (pktType == hPi4Global.CES_CMDIF_TYPE_LOG_IDX) {
        ...
        print('HRV Recordings: Received index - Session ID: $logFileID, Length: $sessionLength samples');

        _logHeaderList.add((logFileID: logFileID, sessionLength: sessionLength));

        if (_logHeaderList.length == _totalSessionCount) {
          print('HRV Recordings: All ${_logHeaderList.length} indices received');
          subscription.cancel();
          _activeSubscriptions.remove(subscription);
          completer.complete();
        }
    ...
    commandPacket.addAll(hPi4Global.HRVRecord);
    await _sendCommand(commandPacket);
    print('HRV Recordings: Sent HRV LogIndex command');

    await completer.future.timeout(
      const Duration(seconds: 30),
      onTimeout: () {
        print('HRV Recordings: ⚠️ TIMEOUT waiting for session indices');
        subscription.cancel();
        _activeSubscriptions.remove(subscription);
    ...
    print('HRV Recordings: Session indices fetch complete. Total headers: ${_logHeaderList.length}');
  }
  ...
  Future<void> _deleteRecording(HRVRecording recording) async {
    ...
    try {
      ...
      await _sendCommand(commandPacket);
      print('HRV Recordings: Sent delete command for session ${recording.sessionId}');
  ...
  /// Wipe all HRV recordings from device
  Future<void> _wipeAllRecordings() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'This will permanently delete all ${_recordings.length} HRV recording(s) from your device. This action cannot be undone.',
    ...
    try {
      ...
      await _sendCommand(commandPacket);
      print('HRV Recordings: Sent wipe all command');
  ...
  /// Download a single HRV recording using FsManager
  Future<void> _downloadRecording(HRVRecording recording) async {
      ...
      downloadSubscription = _fsManager!.downloadCallbacks.listen((event) {
        if (event.path == recording.filePath) {
          ...
          if (event is mcumgr.OnDownloadCompleted) {
            ...
            print('HRV Download: Progress event received (no accessible progress data)');
            ...
            if (mounted) {
              setState(() {
                ...
                print('HRV Download: Estimated progress ${(recording.downloadProgress * 100).toStringAsFixed(0)}%');
              });
            }
          }
        }
  ...
  Future<void> _exportToCsv(HRVRecording recording, List<int> binaryData) async {
    print('HRV Export: Binary data length: ${binaryData.length} bytes');
    print('HRV Export: Expected session length: ${recording.sessionLength} bytes');

    // FsManager downloads raw files from device filesystem - no packet type byte or header
    // Data starts immediately with HRV samples (Int32 little-endian)
    ...
    // Create CSV content - match archived format (HRV values only, no time column)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('HRV Recordings'),
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                'Record HRV data on your device to see it here',
  ...
  Widget _buildRecordingCard(HRVRecording recording) {
    return Card(
