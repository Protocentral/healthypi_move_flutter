...
- Ensure `packageName` in the workflow matches the `applicationId` in `android/app/build.gradle.kts` (currently `com.protocentral.move`).
...
- The workflow assumes a `fastlane` configuration exists under `ios/fastlane` and there's a `release` lane that uploads to App Store Connect. If not present, initialize Fastlane in `ios` and add a `release` lane.
...
- This repository's Flutter code lives at the repository root — all build commands run from there.
