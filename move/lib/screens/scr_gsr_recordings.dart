...
class GSRRecording {
  ...
  String get dateTime {
    ...
    print('GSR Recording: Session $sessionId timestamp: ${timestamp.toIso8601String()} (year: ${timestamp.year})');
  ...
  String get durationText {
    ...
    final sampleCount = sessionLength ~/ GSRConstants.bytesPerSample;
    final durationSeconds = (sampleCount / GSRConstants.samplingRateHz).toInt();
    return '$durationSeconds seconds • ${_formatSampleCount(sampleCount)} samples';
  }

  static String _formatSampleCount(int count) {
    if (count >= GSRConstants.sampleCountThreshold) {
      return '${(count / GSRConstants.sampleCountThreshold).toStringAsFixed(1)}k';
    }
...
class ScrGSRRecordings extends StatefulWidget {
  ...
  const ScrGSRRecordings({super.key, required this.deviceMacAddress});

  @override
  State<ScrGSRRecordings> createState() => _ScrGSRRecordingsState();
}

class _ScrGSRRecordingsState extends State<ScrGSRRecordings> {
  BluetoothDevice? _device;
  mcumgr.FsManager? _fsManager;
  BluetoothCharacteristic? _commandCharacteristic;
  BluetoothCharacteristic? _dataCharacteristic;

  List<GSRRecording> _recordings = [];
  ...
  Future<void> _loadRecordingsList() async {
    ...
    try {
      ...
      // Convert to GSRRecording objects
      final recordings = <GSRRecording>[];
      print('GSR Recordings: Converting ${_logHeaderList.length} headers to recording objects...');

      for (final header in _logHeaderList) {
        // sessionLength is the number of data bytes (no header bytes to subtract)
        // Calculate sample count: dataBytes / bytesPerSample
        final sampleCount = header.sessionLength ~/ GSRConstants.bytesPerSample;
        final durationSeconds = (sampleCount / GSRConstants.samplingRateHz).toInt();
        ...
        print('GSR Recordings:   - Session ${header.logFileID}: ${header.sessionLength} bytes = $sampleCount samples = ${durationSeconds}s');
        print('GSR Recordings:     Timestamp: ${header.logFileID}s → ${dt.toIso8601String()} (year: ${dt.year})');

        recordings.add(GSRRecording(
      ...
      print('GSR Recordings: Recordings sorted. Displaying ${recordings.length} card(s)');
  ...
  Future<void> _fetchSessionCount() async {
    print('GSR Recordings: Fetching session count...');
    ...
    subscription = _dataCharacteristic!.onValueReceived.listen((value) {
      ...
      if (pktType == hPi4Global.CES_CMDIF_TYPE_CMD_RSP) {
        ...
        if (trendCode == hPi4Global.GSRRecord[0]) {
          _totalSessionCount = bdata.getUint16(3, Endian.little);
          print('GSR Recordings: Session count received: $_totalSessionCount');
          subscription.cancel();
    ...
    await completer.future.timeout(
      ...
      onTimeout: () {
        print('GSR Recordings: ⚠️ TIMEOUT waiting for session count');
        subscription.cancel();
        _activeSubscriptions.remove(subscription);
  ...
  Future<void> _fetchSessionIndices() async {
    if (_totalSessionCount == 0) {
      print('GSR Recordings: No sessions to fetch (count = 0)');
      return;
    }

    print('GSR Recordings: Fetching indices for $_totalSessionCount session(s)...');
    ...
    subscription = _dataCharacteristic!.onValueReceived.listen((value) {
      ...
      if (pktType == hPi4Global.CES_CMDIF_TYPE_LOG_IDX) {
        ...
        print('GSR Recordings: Received index - Session ID: $logFileID, Length: $sessionLength samples');

        _logHeaderList.add((logFileID: logFileID, sessionLength: sessionLength));

        if (_logHeaderList.length == _totalSessionCount) {
          print('GSR Recordings: All ${_logHeaderList.length} indices received');
          subscription.cancel();
          _activeSubscriptions.remove(subscription);
          completer.complete();
        }
    ...
    await completer.future.timeout(
      ...
      onTimeout: () {
        print('GSR Recordings: ⚠️ TIMEOUT waiting for session indices');
        subscription.cancel();
        _activeSubscriptions.remove(subscription);
    ...
    print('GSR Recordings: Session indices fetch complete. Total headers: ${_logHeaderList.length}');
  }
  ...
  Future<void> _deleteRecording(GSRRecording recording) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Are you sure you want to delete this GSR recording? This action cannot be undone.',
    ...
    try {
      ...
      await _sendCommand(commandPacket);
      print('GSR Recordings: Sent delete command for session ${recording.sessionId}');
  ...
  Future<void> _wipeAllRecordings() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'This will permanently delete all ${_recordings.length} GSR recording(s) from your device. This action cannot be undone.',
    ...
    try {
      ...
      await _sendCommand(commandPacket);
      print('GSR Recordings: Sent wipe all command');
      ...
      downloadSubscription = _fsManager!.downloadCallbacks.listen((event) {
        if (event.path == recording.filePath) {
          ...
          if (event is mcumgr.OnDownloadCompleted) {
            ...
            print('GSR Download: Progress event received (no accessible progress data)');
            ...
            if (mounted) {
              setState(() {
                ...
                print('GSR Download: Estimated progress ${(recording.downloadProgress * 100).toStringAsFixed(0)}%');
              });
            }
          }
        }
  ...
  Future<void> _exportToCsv(GSRRecording recording, List<int> binaryData) async {
    ...
    final numSamples = cleanData.length ~/ GSRConstants.bytesPerSample;
    ...
    print('GSR Export: Generated ${csvRows.length - 1} CSV rows');
  ...
  Widget _buildRecordingCard(GSRRecording recording) {
    return Card(
